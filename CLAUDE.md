...
`gate → profile (nếu profile_required) hoặc survey (nếu is_first_attempt, plan_type khác) → start → quiz → result`:

1. **gate**: nhập access code (tự điền từ `?code=`), gọi `POST /api/verify-code`. Chặn lại nếu `valid:false` (hiện message theo `reason`), thiếu `exam_type`, hoặc `plan_type` không thuộc whitelist FULL/TRIAL nào (xem mục "Chế độ FULL / TRIAL"). Thành công → `loadExamData(exam_type)` → set `examMode` → nếu `plan_type` thuộc `MANDATORY_PROFILE_PLAN_TYPES` (`one_time`/`subscription`) thì chuyển `profile` (nếu `profile_required`) hoặc thẳng `start` (nếu đã có đủ profile từ trước); ngược lại (trial/staff) chuyển `survey` hoặc `start` tùy `is_first_attempt` như cũ.
2. **profile** (bắt buộc, KHÔNG thể bỏ qua — chỉ áp dụng cho `plan_type` `one_time`/`subscription` khi `verify-code` trả `profile_required:true`, 2026-07-23): form 4 trường bắt buộc — お名前 (text), 国籍/年代/お住まい (3 dropdown, tái dùng `NATIONALITIES`/`AGE_RANGES`/`PREFECTURES` y hệt màn survey), cộng email hiển thị readonly (từ `verifyInfo.email` — field này **chỉ có trong response khi `profile_required:true`**, xem mục API contract) và checkbox opt-in tùy chọn. Nút "送信して次へ" disable tới khi đủ 4 trường bắt buộc; `submitMandatoryProfile()` POST `{access_code, name, nationality, age_range, region, marketing_opt_in}` tới `${API_BASE}/api/access-codes/profile` **ngay lập tức** (không đợi nộp bài thi — để không mất dữ liệu nếu người dùng bỏ ngang), thành công mới chuyển `start`; thất bại hiện lỗi, không cho qua. Lý do tồn tại: với gói trả phí, Stripe Checkout chỉ chắc chắn có email — `name` có thể rỗng, `nationality`/`age_range`/`region` không được thu thập gì cả trong luồng mua hàng. Màn `survey` cũ (tùy chọn, chỉ 3/4 trường, không có `name`) **không áp dụng** cho 2 plan_type này nữa — bị thay thế hoàn toàn bởi màn `profile` mới.
3. **survey** (chỉ hiện lần đầu, có thể bỏ qua — chỉ còn áp dụng cho `trial`/`staff`, xem mục 2): 3 dropdown — 年代 (`AGE_RANGES`), 国籍 (`NATIONALITIES`), お住まい (`PREFECTURES`, 47 都道府県 + 海外) — cộng checkbox opt-in mặc định tắt. Bỏ qua → không gửi field `survey` trong kết quả.
4. **start**: chỉ hiển thị `display_name` (từ verify-code, không cho sửa) — không còn ô nhập nào (email đã bị loại bỏ khỏi màn này, xem mục API contract; với `one_time`/`subscription` email chỉ xuất hiện tạm thời ở màn `profile` phía trên, không lặp lại ở đây). Nếu `examMode==="trial"`, thêm panel so sánh Trial vs Đề thật (xem "Chế độ FULL / TRIAL"). `startExam()` gọi `buildTrialExam()` (trial) hoặc `buildExam()` (full) tùy `examMode`, không cần validate gì thêm.
5. **quiz**: đếm ngược `EXAM_MINUTES` phút (30 phút nếu trial, 70 phút nếu full), tự nộp khi hết giờ; lưu lựa chọn vào `answers` theo `q.id`. Mỗi câu có nhãn loại (`typeLabel(q.type)`) cạnh sect/điểm.
6. `finishExam()`: chấm điểm, tạo `result`, chuyển **result**, gọi `sendResult(res)`. Không phân biệt `examMode` bên trong hàm này.
7. **result**: nếu `examMode==="full"` — điểm theo sect, đậu/rớt, đúng/sai từng câu kèm giải thích, `attempt_number`/`remaining_attempts` từ response exam-results, y hệt trước khi có trial. Nếu `examMode==="trial"` — bảng điểm chi tiết theo phần (`sectPartScoreBreakdown()`, xem "Chế độ FULL / TRIAL"), không có đậu/rớt, khối tĩnh giới thiệu cấu trúc đề thật + disclaimer, nút "有料プランで対策する" (`href={upgradeUrl(verifyInfo.exam_type)}` — trỏ sang `${API_BASE}/upgrade?exam_type=...` bên corporate-site, xem `corporate-site/CLAUDE.md` mục "/upgrade"; đã thay thế `UPGRADE_URL_PLACEHOLDER` cũ, 2026-07-23); phần "解説" (đáp án đúng + giải thích từng câu, kèm nhãn loại câu) dùng chung với full. Nút quay lại: **full** → `handleRetake()` → quay lại **gate** (nhập lại code, `buildExam()` rút đề mới — xem "Chế độ FULL / TRIAL"). **trial** → `handleRetakeSameExam()` → thi lại **chính bộ đề vừa nộp** (xem mục "Chống mất kết quả khi submit fail" bên dưới, phần "もう一度 cho trial").
...
- Request: `{ access_code }`
- Hợp lệ (`valid:true`, HTTP 200): `{ valid, display_name, exam_type, remaining_attempts, expires_at, plan_type, is_first_attempt, profile_required, email? }`
  ...
  - **`profile_required` (boolean, thêm 2026-07-23)**: true khi `plan_type` ∈ `MANDATORY_PROFILE_PLAN_TYPES` (`one_time`/`subscription`) **và** user đó còn thiếu ít nhất 1 trong 4 trường name/nationality/age_range/region. `trial`/`staff` luôn `false`. FE dùng field này để quyết định có chuyển sang `screen==="profile"` hay không (xem mục "Flow thi").
  - **`email` (thêm 2026-07-23, CHỈ xuất hiện khi `profile_required===true`)**: ngoại lệ có chủ đích của nguyên tắc cũ "response không bao giờ chứa email" — chỉ lộ ra đúng lúc cần hiển thị readonly ở màn `profile`, không có ở mọi trường hợp khác (kể cả cùng access_code đó sau khi đã hoàn tất profile).
...
**`POST /api/access-codes/profile`** (mới, 2026-07-23 — xem mục "Flow thi" bước 2 cho luồng gọi)
- Request: `{ access_code, name, nationality, age_range, region, marketing_opt_in? }` — 4 trường đầu bắt buộc non-empty, `nationality`/`age_range`/`region` phải khớp đúng 1 trong các giá trị `NATIONALITIES`/`AGE_RANGES`/`PREFECTURES` (cùng danh sách dùng cho màn `survey`).
- Chấp nhận (`accepted:true`, HTTP 200): ghi đè trực tiếp (không phải `coalesce`) 4 trường tương ứng trên `users` phía corporate-site.
- Từ chối (`accepted:false`, HTTP 200 hoặc 400): `{ accepted:false, reason }`, `reason` ∈ `"invalid_request"` (thiếu/sai định dạng trường, HTTP 400) | `"not_found"` (access_code không tồn tại) | `"plan_type_mismatch"` (access_code không phải `one_time`/`subscription` — phòng vệ nhiều lớp phía server, phòng trường hợp bị gọi trực tiếp bỏ qua điều kiện phía FE). FE không retry tự động khi thất bại — chỉ hiện lỗi, giữ nguyên màn `profile`, người dùng bấm gửi lại.
